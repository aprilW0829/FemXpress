...
parser.add_argument("-p","--parents_genotypes", type=str, help="information of parents' genotypes")
...
#print(args.annot)
#print(args.sample)
#print(args.parents_genotypes)

inference_dir='./FemXpress/'+args.sample+'/inference'
cmd = 'cd {}'.format(inference_dir)
print('[FemXpress process]'+'\t'+cmd)
os.system(cmd)
...
for dir_path in dir_paths:
    if not os.path.exists(dir_path):
        ...
        print('[FemXpress process] create inference dir!')
    else:
        print('[FemXpress process] inference dir already exist!')
...
fout_escape_3 = open(inference_dir+"/escape_pos_list_method_3_pos.txt", "w")
fout_escape_log = open(inference_dir+"/escape_pos_list_method_3_pos.log", "w")
...
LINKAGE_CANDIDATE_NUM = 10000000
...
def find_best_position(candidate_position_list):
    print("[FemXpress process] Finding best starting cutting point ...")
    max_score = 0
...
def find_random_position(candidate_position_list):
    print("[FemXpress process] Finding random OK starting cutting point ...")
    base_1 = ""
    base_2 = ""
    while True:
            ...
            for base in position_dict[pos].keys():
                if len(position_dict[pos][base]) > MINIMUM_DIVIDED_CELL_COUNT:
                    if base_1 == "":
                        ...
                        base_2 = base
                        print("[FemXpress process]", pos, base_1, base_2)
...
def merge_all_genotypes(genotype_list):
    ...
    print("[FemXpress process] test_merge_list: "+str(test_merge_list(cell_set_list)))
    while test_merge_list(cell_set_list):
        ...
        for i in range(len(new_cell_set_list)-1):
            print("[FemXpress process] Merge starts from "+str(i))
                ...
                while j < len(new_cell_set_list):
                    if new_cell_set_list[j][0] != set() and new_cell_set_list[j][1] != set():
                        if test_merge_two_divisions(new_cell_set_list[i][0], new_cell_set_list[i][1], new_cell_set_list[j][0], new_cell_set_list[j][1]) != 0:
                            print("[FemXpress process] Merge "+str(i)+" and "+str(j))
    ...
    while i<len(cell_set_list):
        if len(cell_set_list[i][0]) > len(cell_set_list[largest_i][0]):
            largest_i = i
        i = i + 1
    print('[FemXpress process]' ,str(largest_i))
    print('[FemXpress process]',str(len(cell_set_list)), str(len(cell_vote_dict_list)))
...
def find_best_divisive_position(cell_set, pos_set):
    ...
    for pos in pos_set:
        ...
        if score > max_score:
            ...
            base_1 = max_len_base_1
            base_2 = max_len_base_2
    print('[FemXpress process]', str(best_pos), base_1, base_2)
    return best_pos, base_1, base_2
...
def plotCurve(x_data, y_data):
    ...
    plt.savefig(inference_dir+"/inflection_v12.png")
...
def readData():
    ...
    pos_escape_2_dict = {}
    i=0
    print("[FemXpress process] Preprocessing the data ...")
    for line in fin.readlines():
        if i==0:
            ...
            for item in sp:
                ...
                k=k+1
            total_snp_count = k
            print("[FemXpress process] Totally found "+str(k)+" SNPs in the sample.")
            if k<COVERAGE_SNP_TO_STOP[0]:
                break
            i=i+1
        else:
            if i%1000==0:
                print("[FemXpress process] Reached the "+str(i)+"th record")
            sp=line.strip().split("\t")
            j=0
            barcode = ""
            for item in sp:
    ...
    print("[FemXpress process] Ratio median: "+str(median))
    sns.displot(ratio_list, kde=True, bins=100)
    plt.axvline(x=median, color="red")
    plt.savefig(inference_dir+"/ratio.png")
    ...
    if COVERAGE_SNP_TO_STOP[0] < total_snp_count * 0.01:
        COVERAGE_SNP_TO_STOP[0] = math.floor(total_snp_count * 0.01)
    ...
    #    percentile = (snp_count/total_snp_count)*100
...
def runPipeline(n):
    ...
    print("[FemXpress process] Total round number: "+str(len(coverage_parameter_list)))
    print("[FemXpress process] Coverage thresholds:"+str(coverage_parameter_list))
    print("[FemXpress process] Base coverage threholds: "+str(base_coverage_parameter_list))
    print("[FemXpress process] Test round "+str(n)+" ...")
    ...
    if TOTAL_READS_AT_POSITION_THRES <= 0 and coverage_parameter_list != []:
        ...
        print("[FemXpress process] TOTAL_READS_AT_POSITION_THRES not set. Automatically set to "+str(coverage_parameter_list[n])+".")
    ...
    if TOTAL_READS_COMFIRM_BASE_THRES <= 0 and base_coverage_parameter_list != []:
        ...
        print("[FemXpress process] TOTAL_READS_COMFIRM_BASE_THRES not set. Automatically set to "+str(base_coverage_parameter_list[n])+".")
    ...
    median = np.percentile(ratio_list, 50)
    print("[FemXpress process] Round median: "+str(median))
    diff = 45
    if MINOR_ALLELE_PERCENTAGE_HIGHER_BOUND <= 0:
        LOCAL_MINOR_ALLELE_PERCENTAGE_HIGHER_BOUND = np.percentile(ratio_list, 50+diff)
        print("[FemXpress process] MINOR_ALLELE_PERCENTAGE_HIGHER_BOUND not set. Automatically set to "+str(LOCAL_MINOR_ALLELE_PERCENTAGE_HIGHER_BOUND)+".")        
    if MINOR_ALLELE_PERCENTAGE_LOWER_BOUND <= 0:
        LOCAL_MINOR_ALLELE_PERCENTAGE_LOWER_BOUND = np.percentile(ratio_list, 50-diff)
        print("[FemXpress process] MINOR_ALLELE_PERCENTAGE_LOWER_BOUND not set. Automatically set to "+str(LOCAL_MINOR_ALLELE_PERCENTAGE_LOWER_BOUND)+".")

    global LINKAGE_TEST_CELL_COUNT_THRES, LOCAL_LINKAGE_TEST_CELL_COUNT_THRES
    if LINKAGE_TEST_CELL_COUNT_THRES <= 0:
        print("[FemXpress process] Setting background linkage parameter ...")
        ...
        print("[FemXpress process] Size of candidate linkage list: "+str(len(total_cells_count_list)))
        ...
        print("[FemXpress process] LOCAL_LINKAGE_CANDIDATE_NUM set to "+str(LOCAL_LINKAGE_CANDIDATE_NUM))
        LINKAGE_PERCENTILE = (1-(LOCAL_LINKAGE_CANDIDATE_NUM/len(total_cells_count_list)))*100
        print("[FemXpress process] Linkage percentile is: "+str(LINKAGE_PERCENTILE))
        ...
        print("[FemXpress process] LINKAGE_TEST_CELL_COUNT_THRES not set. Automatically set to: "+str(LOCAL_LINKAGE_TEST_CELL_COUNT_THRES))
    else:
        LOCAL_LINKAGE_TEST_CELL_COUNT_THRES = LINKAGE_TEST_CELL_COUNT_THRES

    print("[FemXpress process] Testing linkages ...")
    good_linkage_list = []
    j=0
    for barcode in cell_dict.keys():
        if j%1000 == 0:
            print("[FemXpress process] Tested "+str(j)+" cells for good linkages candidates ...")
        positions = sorted(list(cell_dict[barcode].keys()), key=lambda x:int(x))
        i=0
        for pos_1 in positions:
    ...
    print("[FemXpress process] Get "+str(len(linkage_dict.keys()))+" positions on chrX.")
    ...
    print("[FemXpress process] Size of cell count dict 1: "+str(len(cell_count_dict_1.keys())))
    print("[FemXpress process] Size of cell count dict 2: "+str(len(cell_count_dict_2.keys())))
...
if total_snp_count<COVERAGE_SNP_TO_STOP[0]:
    print("[FemXpress process] Total SNP number is less than "+str(total_snp_count)+". The sample is not classified.")
else:
    ...
    if dict_index == len(PERCENTILES):
        print("[FemXpress process] The sample is not classified.")
    else:
        print("[FemXpress process] Percentile "+str(PERCENTILES[dict_index])+" as reference.")
        ...
        while j<len(PERCENTILES):
            ...
            j=j+1
    #ari_str = ','.join(ari_list)
    #print('[FemXpress process] ', ari_str)
    ...
    print("[FemXpress process] best_n: "+str(best_n))
    ...
    # Determine cells' parental origin
    # parents_phenotypes.tsv format:
    # [TAB]paternal_sample_name[TAB]maternal_sample_name[Enter]
    # chrX_pos_1(e.g., 10021)[TAB]paternal_allele(s)(e.g., C)[TAB]maternal_allele(s)(e.g., AA)[Enter]
    # chrX_pos_2(e.g., 10028)[TAB]paternal_allele(s)(e.g., CA)[TAB]maternal_allele(s)(e.g., AT)[Enter]
    # ...
    #if os.path.isfile("parents_genotypes.tsv"):
    if os.path.exists(args.parents_genotypes): # xiugai
        #f_parents = open("parents_genotypes.tsv", "r")
        f_parents = open(args.parents_genotypes, 'r') # xiugai
        f_parents_corr = open(inference_dir+"/parental_origin.txt", "w")
        i = 0
        f_parents_list = f_parents.readlines()
        p_name = "paternal"
        m_name = "maternal"
        # parent_pos_dict[chrX_pos] = [pat_allele, mat_allele]
        parent_pos_dict = {}
        while i < len(f_parents_list):
            if i == 0:
                sp = f_parents_list[i].strip().split("\t")
                p_name = sp[0]
                m_name = sp[1]
            else:
                sp = f_parents_list[i].strip().split("\t")
                ...
                p_set = father_set.intersection(symmetric_diffrence_set)
                m_set = mother_set.intersection(symmetric_diffrence_set)
                if len(p_set)==1 and len(m_set)==1:
                    parent_pos_dict[sp[0]] = [p_set.pop(), m_set.pop()]
            i = i + 1
        # parent_predicted_pos_dict[chrX_pos] = [allele_1, allele_2]
        parent_predicted_pos_dict = {}
        for line in all_genotypes[best_n]:
            sp = line.strip().split("&")
            g1 = sp[0]
            g2 = sp[1]
            ...
            for i in range(len(sp1)):
                spsp1 = sp1[i].strip().split(" ")
                spsp2 = sp2[i].strip().split(" ")
                parent_predicted_pos_dict[spsp1[0]] = [spsp1[1], spsp2[1]]
        evi_00 = 0
        evi_01 = 0
        evi_10 = 0
        evi_11 = 0
        for pos in parent_predicted_pos_dict.keys():
            if pos in parent_pos_dict.keys():
                pat_allele = parent_pos_dict[pos][0]
                mat_allele = parent_pos_dict[pos][1]
                allele_1 = parent_predicted_pos_dict[pos][0]
                allele_2 = parent_predicted_pos_dict[pos][1]
                if pat_allele == allele_1:
                    evi_00 = evi_00 + 1
                if pat_allele == allele_2:
                    evi_01 = evi_01 + 1
                if mat_allele == allele_1:
                    evi_10 = evi_10 + 1
                if mat_allele == allele_2:
                    evi_11 = evi_11 + 1
        if evi_00 + evi_11 >= evi_01 + evi_10:
            f_parents_corr.write("In the clustering results:\n")
            f_parents_corr.write("0 represents "+p_name+"\n")
            f_parents_corr.write("1 represents "+m_name+"\n")
        else:
            f_parents_corr.write("In the clustering results:\n")
            f_parents_corr.write("0 represents "+m_name+"\n")
            f_parents_corr.write("1 represents "+p_name+"\n")
        f_parents.close()
        f_parents_corr.close()

    cluster_result = method_1_all_results[best_n]
    qualified_set_1 = set()
    qualified_set_2 = set()
    for pos in pos_list:
        best_pos, base_1, base_2 = find_two_alleles(pos)
        fout_escape_log.write(str(best_pos)+"\tbase1: "+base_1+"\tbase2: "+base_2+"\n")
        if base_1 != "" and base_2 != "":
            ...
            for base in pos_base_count_dict_1.keys():
                fout_escape_log.write("Cluster_1: "+str(pos)+"\t"+base+"\t"+str(pos_base_count_dict_1[base])+"\n")
            for base in pos_base_count_dict_2.keys():
                fout_escape_log.write("Cluster_2: "+str(pos)+"\t"+base+"\t"+str(pos_base_count_dict_2[base])+"\n")
            if len(pos_base_count_dict_1.keys()) != 0:
                # sum_count_1 = 0            
                # for base in pos_base_count_dict_1.keys():
                #     sum_count_1 = sum_count_1 + pos_base_count_dict_1[base]
                
                if base_1 in pos_base_count_dict_1.keys() and base_2 in pos_base_count_dict_1.keys():
                    sum_count_1 = pos_base_count_dict_1[base_1] + pos_base_count_dict_1[base_2]
                    per_1 = pos_base_count_dict_1[base_1]/sum_count_1
                    per_2 = pos_base_count_dict_1[base_2]/sum_count_1
                
                    if (per_1 >= per_2 and per_1 >= 0.3 and per_1 <= 0.7 and pos_base_count_dict_1[base_1] >= 5) or (per_2 >= per_1 and per_2 >= 0.3 and per_2 <= 0.7 and pos_base_count_dict_1[base_2] >= 5):
                    ...
                    #         qualified_set_1.add(pos)
                    #         break
                    fout_escape_log.write(str(pos)+"\t"+base_1+"\t"+str(pos_base_count_dict_1[base_1])+"\t"+str(per_1)+"\t"+base_2+"\t"+str(pos_base_count_dict_1[base_2])+"\t"+str(per_2)+"\n")
            if len(pos_base_count_dict_2.keys()) != 0:
                # sum_count_2 = 0
                # for base in pos_base_count_dict_2.keys():
                #     sum_count_2 = sum_count_2 + pos_base_count_dict_2[base]
                if base_1 in pos_base_count_dict_2.keys() and base_2 in pos_base_count_dict_2.keys():
                    sum_count_2 = pos_base_count_dict_2[base_1] + pos_base_count_dict_2[base_2]
                    per_1 = pos_base_count_dict_2[base_1]/sum_count_2
                    per_2 = pos_base_count_dict_2[base_2]/sum_count_2
                    if (per_1 >= per_2 and per_1 >= 0.3 and per_1 <= 0.7 and pos_base_count_dict_2[base_1] >= 5) or (per_2 >= per_1 and per_2 >= 0.3 and per_2 <= 0.7 and pos_base_count_dict_2[base_2] >= 5):
                    ...
                    #         qualified_set_2.add(pos)
                    #         break
                    fout_escape_log.write(str(pos)+"\t"+base_1+"\t"+str(pos_base_count_dict_2[base_1])+"\t"+str(per_1)+"\t"+base_2+"\t"+str(pos_base_count_dict_2[base_2])+"\t"+str(per_2)+"\n")

    qualified_intersection = qualified_set_1.union(qualified_set_2)
    ...
    print("[FemXpress process] Ratio of each round:")
    ...
    for round_i in range(len(all_genotypes)):
        print("[FemXpress process] Genotype of round "+str(round_i))
        for line in all_genotypes[round_i]:
            print('[FemXpress process] ' + line.strip())
...
#fout_escape_2.close()
fout_escape_3.close()
fout_escape_log.close()
print('[FemXpress process] FemXpress inference analysis, done!')
