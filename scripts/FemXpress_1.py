...
#sample_name=sys.argv[1][0:-25]
#print(sample_name)
#os.mkdir(sample)
#os.chdir(sample_name)

tmp_dir='./FemXpress/tmp'
result_dir='./FemXpress/result'
inter_base_dir='./FemXpress/base_count'
...
#### def get all files under the dir
all_d_snp_loci_dic={}
sgtxt_list=[]
...
subprocess.run(["samtools", "index", output_bam, "-@ 8"])
...
for barcode in barcode_list:
    ###search for first location (up than AGCT four bases)
    for i in range(len(barcode)-16):
        if set(barcode[i:i+16]).issubset({'A','G','C','T'}):
            start_pos=i
            break
    #start_index = barcode.find('AGCT')
    #if start_index == -1:
    #    continue  # 如果该Barcode不包含AGCT，则跳过处理
    # 提取从AGCT开始到字符为-1的字符串
    if start_pos is not None:
        #end_pos = barcode.find("-1", start_pos)
        end_pos=-1
        #end_index = barcode.find('-')
    #if end_index == -1:
    #    end_index = len(barcode)
    #trimmed_barcode = barcode[start_index:end_index]
    trimmed_barcode = barcode[start_pos:]
    # 打印处理后的结果
    print(trimmed_barcode)
    fo_tmp1.write(trimmed_barcode+'\n')
    trimmed_barcode_list.append(trimmed_barcode)
#print(trimmed_barcode_list)
print('add all barcodes to meta info,done!')


#### get filtered_bam of picked barcodes
#input_bam=output_bam #(all_chrx_bam)
input_bam=tmp_dir+'/output_chrX.bam'
output_bam1=tmp_dir+'/filtered_chrX.bam'
...
### get retained snp for filtering vcf
...
# chrX    2156    2157    agaccagcccggccaacatgg
## def input genome
...
print(simple_repeat_snp_dic)
print('get simple repeat ssequence of mm10"s chrX of mm10 done!')


# get up and down 20bp bed location and sequence of the SNP (filter simple_repeat_sequence :yes and or)
snp_updown20bp_bed_sequence_ml_dic={}
...
def get_up_down20bp_bed_sequence_dic(f,genome1,simple_repeat_snp_dic):
    for i in f.readlines():
        ...
        if snp_loci in simple_repeat_snp_dic.keys():
            print(snp_loci)
            pass
        else:
    ...
    return snp_updown20bp_bed_sequence_dic,snp_updown20bp_bed_sequence_ml_dic
...
all_base_dic={}
all_base_list=[]
for i in snp_updown20bp_bed_sequence_dic.keys():
    x=i
    i=i.split('\t')
...
print('increase number of opened txt done!!')

#### get single barcode of filtered chrX.bam
output_bam=tmp_dir+'/filtered_chrX.bam'
output_dir = inter_base_dir
with pysam.AlignmentFile(output_bam, "rb",ignore_truncation=True) as bam_file:
    barcode_files = {}
...
for bam_file in bam_files:
    ...
    barcode=bam_file[:-4]
    command = f"/data2/wangxin/biosoft/bam-readcount/build/bin/bam-readcount -w 0 -f {reference_fa} {bam_file1} -l {bed_file2} | awk 'BEGIN{{FS=OFS=\"\t\"}}{{split($6,A,\":\");split($7,C,\":\");split($8,G,\":\");split($9,T,\":\");print $1\";\"$2,A[2]\";\"C[2]\";\"G[2]\";\"T[2],$3,$4}}' > {inter_base_dir}/{barcode}_base_count.txt"
...
############################################ step3:
### add col index for each barcode's count txt and all snp loci
data = []
positions=[]
...
snp_loci_list1=[]
snp_loci_list2=[]
def define_filtered_snp_loci(i):
    i=i.split('\t')
    snp_loci=i[0]+';'+i[1]
    snp_loci_dic[snp_loci]=1
    snp_loci_list.append(snp_loci)

# chrX    16066   16067   CGGGGAGCTTGGCTCTTCAGG
# chrX    233757  233758  GGCCAACACAGTGAAACCCCA
f=open(tmp_dir+'/filtered_freebayes_snps_manual_info.txt','r')
for i in f:
    define_filtered_snp_loci(i)
dic1=snp_loci_dic
snp_loci_list1=snp_loci_list
#print('get loci of filtered by manual parameters done!')
...
#print(len(snp_loci_list2))

print('get loci of filtered freebayes"s snps done!')
...
def get_single_snp_coverage(txt_list,snp_loci_dic):
    for i in txt_list:
        barcode=i[:18]
    ...
    return each_snp_info_dic,snp_loci_4base_barcodes_list_dic
...
get_single_snp_coverage(sgtxt_list,dic1)
...
#### get two main bases's distribution of each snp (unfiltered)
value_list=[]
value_ratio1=[]
value_ratio1_dic={}
value_ratio2=[]
...
# chrX;13627143   1.6666666666666667      0       10      0       6
# chrX    4060098 4060099 CACTGGCTTGGTCAGCTTGTT
# chrX    4060222 4060223 TTTCTCTGTATAGCCCTGGCT
snp_sequence_dic={}
dfi=open(tmp_dir+'/filtered_freebayes_snps_manual_info.txt','r')
for i in dfi:
    i=i.split('\t')
    snp_loci=i[0]+';'+i[1]
...
lower = np.percentile(value_ratio1, 10)
upper = np.percentile(value_ratio1, 90)
for a in value_ratio1_dic.keys():
    if float(value_ratio1_dic[a].split('\t')[0]) > lower and float(value_ratio1_dic[a].split('\t')[0]) < upper :
        value_ratio2.append(float(value_ratio1_dic[a].split('\t')[0]))
        value_ratio2_dic[a]=value_ratio1_dic[a]
    else:
        pass
for a in value_ratio3_dic.keys():
    if float(value_ratio3_dic[a].split('\t')[0]) > lower and float(value_ratio3_dic[a].split('\t')[0]) < upper :
        value_ratio4.append(float(value_ratio3_dic[a].split('\t')[0]))
        value_ratio4_dic[a]=value_ratio3_dic[a]
    else:
        pass

for i in value_ratio1_dic.keys():
    #print(str(value_ratio1_dic[i]))
    dfo_inter1.write(i+'\t'+str(value_ratio1_dic[i])+'\n')
for i in value_ratio2_dic.keys():
    #print(str(value_ratio2_dic[i]))
    dfo_inter2.write(i+'\t'+str(value_ratio2_dic[i])+'\n')
for i in value_ratio3_dic.keys():
    #print(str(value_ratio3_dic[i]))
    dfo_inter3.write(i+'\t'+str(value_ratio3_dic[i])+'\n')
for i in value_ratio4_dic.keys():
    #print(str(value_ratio4_dic[i]))
    dfo_inter4.write(i+'\t'+str(value_ratio4_dic[i])+'\n')

### visulization
log2_unfiltered_list = [math.log2(x) for x in value_ratio1]
log2_filtered_ratio2_list = [math.log2(x) for x in value_ratio2]
log2_filtered_ratio3_list = [math.log2(x) for x in value_ratio3]
log2_filtered_ratio4_list = [math.log2(x) for x in value_ratio4]
...
plt.hist(value_ratio3,density=True,bins=40)
...
plt.hist(value_ratio1,density=True,bins=40)
...
## filtered by ratio
plt.hist(value_ratio2,density=True,bins=40)
...
plt.hist(value_ratio4,density=True,bins=40)
...
print('number of unfiltered snp is ',str(len(value_ratio1)))
print('get distribution of unfiltered snp done!')
print('number of filtered snp by ratio is ',str(len(value_ratio2)))
print('get distribution of filtered snp by ratio done!')
print('number of filtered snp by reads is ',str(len(value_ratio3)))
print('get distribution of filtered snp by reads done!')
print('number of filtered snp by ratio and reads is ',str(len(value_ratio4)))
print('get distribution of filtered snp by ratio and reads done!')


################################################# step5:
#################################################### output :merged snp-barcode matrix
#### merge all snp base info of each barcode
...
final_path = []
for i in sgtxt_list:
    tmp_final_path = os.path.join(path, i)
...
result = pd.concat([df[column] for df in dfs_flat], axis=1)
data = result
...
###################################################### step6:
####  create matrix1_supple after base filtering
...
df_filter2 = pd.read_csv(tmp_dir+'/each_snp_coverage_barcodes_filtered_by_reads_ratio.txt',sep='\t',header=None, names=['Chromosome;Position', 'ratio','A','C','F','T'])
df_filter3 = pd.read_csv(tmp_dir+'/each_snp_coverage_barcodes_filtered_by_ratio_ratio.txt',sep='\t',header=None, names=['Chromosome;Position', 'ratio','A','C','F','T'])
df_filter4 = pd.read_csv(tmp_dir+'/each_snp_coverage_barcodes_filtered_by_read_and_ratio_ratio.txt',sep='\t', header=None, names=['Chromosome;Position', 'ratio','A','C','F','T'])
...
csv_path = result_dir+'/result_matrix1_1.csv'
...
filtered_df2_re = filtered_df2.replace('\t', '.;.;.;.')
# matrix3
filtered_df3 = df[filter_list3]
filtered_df3_re = filtered_df3.replace('\t', '.;.;.;.')
# matrix4
filtered_df4 = df[filter_list4]
filtered_df4_re = filtered_df4.replace('\t', '.;.;.;.')

row_names = df.index.tolist()
filtered_df2_re.index = row_names
filtered_df3_re.index = row_names
filtered_df4_re.index = row_names
#print('get filtered barcode-snp matrix2,matrix3,matrix4 done!')
#print(filtered_df2_re.index[:6])
#print(filtered_df2_re.shape)
#print(filtered_df3_re.shape)
#print(filtered_df4_re.shape)
print('dims for barcode-snp matrix2 is ',str(filtered_df2_re.shape))
print('dims for barcode-snp matrix3 is ',str(filtered_df3_re.shape))
print('dims for barcode-snp matrix4 is ',str(filtered_df4_re.shape))

filtered_df2_re.to_csv(result_dir+'/result_matrix2.csv',sep='\t',mode='w',header=True,index=True)
filtered_df3_re.to_csv(result_dir+'/result_matrix3.csv',sep='\t',mode='w',header=True,index=True)
filtered_df4_re.to_csv(result_dir+'/result_matrix4.csv',sep='\t',mode='w',header=True,index=True)
print('get filtered barcode-snp matrix2,matrix3,matrix4 done!')
